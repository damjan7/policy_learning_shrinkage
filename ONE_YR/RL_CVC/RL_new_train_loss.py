...
def train_with_dataloader(normalize=False):
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for i, data in enumerate(train_dataloader):
            ...
            optimizer.zero_grad()
            cvc_shrk = X[0][-1] / 100
            possible_shrkges = torch.tensor([i/100 for i in range(101)])
            loss = criterion(out, labels) + criterion(cvc_shrk * 1000, possible_shrkges * 1000)  # MSE between outputs of NN and pf std --> pf std can be interpreted
        ...
        with torch.no_grad():
            for i, data in enumerate(val_dataloader):
                ...
                dst = torch.abs(dst - X[0][-1]/100) * (0.00) # even smaller, since very few stocks
