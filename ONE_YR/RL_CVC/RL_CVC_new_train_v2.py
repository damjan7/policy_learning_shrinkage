...
if create_new_inputs == True:
    ...
    ni_df.to_csv(new_inputs_folder + r"\new_inputs.csv", index=False)
    print("created new inputs")
else:
    new_inputs = pd.read_csv(new_inputs_folder + r"\new_inputs.csv").to_dict(orient='list')

smooth_cvc_shrinkages = True
if smooth_cvc_shrinkages == True:
    ...
    optimal_shrk_data['shrk_factor'] = x4_smooth
...
class MyDataset(Dataset):
    def __init__(self, factors, ew_year_vola, ew_month_vola, trace,
                 q075, q025, all_yr_vola, all_month_vola,
                 fixed_shrk_data, optimal_shrk_data, normalize=False):
        if normalize == True:  # for now only scale factors, I don't scale them actually
            ...
            self.ew_year_vola = ew_year_vola
            self.ew_month_vola = ew_month_vola
            self.trace = trace
            self.q075 = q075
            self.q025 = q025
            self.all_yr_vola = all_yr_vola
            self.all_month_vola = all_month_vola
            self.factors = factors
            print("loaded")

    def __len__(self):
        return len(self.ew_year_vola)

    def __getitem__(self, idx):
        # inputs multiplied by 100 works better
        inp = torch.Tensor( (
            self.ew_year_vola[idx], self.ew_month_vola[idx], self.trace[idx], self.q075[idx],
            self.q025[idx], self.all_yr_vola[idx], self.all_month_vola[idx],
...
def train_with_dataloader(normalize=False, dst=None, batch_size=None):
    ...
    train_dataset = MyDataset(
        ...
        new_inputs['EW_Year_vola'][:len_train],
        new_inputs['EW_Month_vola'][:len_train],
        new_inputs['sample_covmat_trace'][:len_train],
        new_inputs['allstocks_q075_month'][:len_train],
        new_inputs['allstocks_q025_month'][:len_train],
        new_inputs['allstocks_year_avgvola'][:len_train],
        new_inputs['allstocks_month_avgvola'][:len_train],
    ...
    for epoch in range(1, num_epochs+1):
        ...
        if epoch in [7, 14, 20, 25, 27, 29, 33, 36, 39]:
...
# PARAMETERS:
num_epochs = 41
num_features = 8  # all 13 factors + opt shrk
