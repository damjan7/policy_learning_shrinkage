...
def train_with_dataloader(normalize=False):
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for i, data in enumerate(train_dataloader):
            ...
            # 10 smallest indices
            n_argmin_train.append(eval_function_new.get_n_smallest_indices(np.array(labels[0].detach()), 15))
        ...
        with torch.no_grad():
            for i, data in enumerate(val_dataloader):
                ...
                dst = torch.abs(dst - X[0][-1]/100) * (0.0) # even smaller, since very few stocks
                ...
                val_preds_v2.append(eval_function_new.get_n_smallest_indices(np.array(labels[0]), 15))
...
cvc_shrk_train = train_dataset.optimal_shrk_data['shrk_factor']

# train eval
eval_function_new.quick_eval_train(len_train, end_date, train_preds, train_dataset, rets_full, permnos, calc_res = True)

# additional stuff
val_preds_n_min = pd.DataFrame([np.mean(i)/100 for i in val_preds_v2])
val_preds_roll_mean_v2 = pd.DataFrame([np.mean(i)/100 for i in val_preds_v2]).rolling(30).mean().dropna()

train_argmin_mean = pd.DataFrame([np.mean(i)/100 for i in val_preds_v2])


argmin_validation = [i/100 for i in actual_argmin_validationset]
r_val1 = eval_function_new.eval_fct_new_1YR(np.array(argmin_validation)[val_idxes_shrkges], rets_full, permnos, 0, val_indices_results)
'''
