...
class MyDataset(Dataset):
    def __init__(self, sample_covmats, fixed_shrk_data, optimal_shrk_data, normalize=False):
        if normalize == True:  # for now only scale factors, I don't scale them actually
            self.factors_scaler = MinMaxScaler()
            self.sample_covmats = pd.DataFrame(self.sample_covmats.fit_transform(factors))
        else:
            self.sample_covmats = sample_covmats
        self.fixed_shrk_data = fixed_shrk_data
        self.optimal_shrk_data = optimal_shrk_data
        print("loaded")

    def __len__(self):
        return len(self.sample_covmats)

    def __getitem__(self, idx):
        # inputs multiplied by 100 works better
        inp = torch.Tensor(self.sample_covmats[idx]) * 100
...
# PARAMETERS:
num_epochs = 41
num_features = sample_covmats[0].shape[0]  # all 13 factors + opt shrk
