...
def create_fixed_shrk_datasets(path, end_date, p, out_pf_sample_period_length, estimation_window_length,
    ...
    df = df.drop(0)
    with open(
            rf"H:\all\RL_Shrinkage_2024\ONE_YR\preprocessing\training_dfs\PF{p}/fixed_shrkges_{estimator.__name__}_p{p}.pickle",
            'wb') as pickle_file:
        pickle.dump(df, pickle_file)

    rawres_df = pd.DataFrame(shrk_rawres_all)
    with open(
            rf"H:\all\RL_Shrinkage_2024\ONE_YR\preprocessing\training_dfs\PF{p}/fixed_shrkges_rawres_{estimator.__name__}_p{p}.pickle",
            'wb') as pickle_file:
