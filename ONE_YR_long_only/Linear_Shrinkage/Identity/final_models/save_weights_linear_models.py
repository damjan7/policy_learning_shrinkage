...
for tmp_pf_size in [30, 50, 100, 225, 500]:
    with open(fr"H:\all\RL_Shrinkage_2024\ONE_YR_long_only\preprocessing\training_dfs\PF{tmp_pf_size}\cov1Para_factor-1.0_p{tmp_pf_size}.pickle", 'rb') as f:
        tmp = pickle.load(f)
...
tmp_res_path = r"H:\all\RL_Shrinkage_2024\ONE_YR_long_only\Linear_Shrinkage\results"
...
out_path = fr"H:\all\RL_Shrinkage_2024\ONE_YR_long_only\Linear_Shrinkage\results\p{pf_size}"
...
daily_rets = pd.DataFrame(daily_rets, index=dates[5040:])
# returns are not correct AFAIK
daily_rets.to_csv(out_path + f"\c1p_p{pf_size}_daily_returns.csv")
optimal_shrk_data['shrk_factor'].to_csv(out_path + f"\c1p_p{pf_size}_intensities.csv")

"""
tmp = []
for PF_SIZE in [30,50,100,225,500]:
    tmp_path = fr"H:\all\RL_Shrinkage_2024\ONE_YR_long_only\Linear_Shrinkage\results\p{PF_SIZE}"
    tmp.append(pd.read_csv(tmp_path + f"\c1p_p{PF_SIZE}_daily_returns.csv", index_col=0).values.reshape(-1))
tmp_df = pd.DataFrame(tmp).T
tmp_df.index = dates[5040:]
tmp_df.columns = [30,50,100,225,500]
tmp_df.to_csv(rf"H:\all\RL_Shrinkage_2024\ONE_YR_long_only\Linear_Shrinkage\results" + rf"\cov1para_daily_returns.csv")
"""
...
for PF_SIZE in [30, 50, 100, 225, 500]:
    in_path = rf"H:\all\RL_Shrinkage_2024\ONE_YR_long_only\Linear_Shrinkage\results\p{PF_SIZE}"
    out_path = in_path
    # load intensities
