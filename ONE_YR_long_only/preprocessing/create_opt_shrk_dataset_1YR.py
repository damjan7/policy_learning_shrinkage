...
def create_fixed_shrk_datasets(path, end_date, p, out_pf_sample_period_length, estimation_window_length,
    ...
    for factor in shrk_factors:
        ...
        for idx in range(rets_full.shape[0]-259-21):
            ...
            pf_ret, pf_std = hf.calc_pf_std_LONG_ONLY_return(covmat_est, future_return_matrix)
        ...
        df = df.drop(0)
        with open(rf"H:\all\RL_Shrinkage_2024\ONE_YR_long_only\preprocessing\training_dfs\PF{p}/{estimator.__name__}_factor-{factor}_p{p}.pickle", 'wb') as pickle_file:
            pickle.dump(df, pickle_file)

estimator = estimators.cov2Para
#pf_size = 500
in_path = r"H:\all\RL_Shrinkage_2024\ONE_YR_long_only\preprocessing\rets_permnos_1Y"
